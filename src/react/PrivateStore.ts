...
import {default as Core, Module, DispatchMutator} from '@neurongsm/core';
import { Selector } from '@neurongsm/slices';
...
export default class PrivateStore<S = {[key:string]: unknown}, M=unknown>{
  ...
  useNeuron = <T = unknown, A = {[key:string]: unknown}>(selector: Core.SelectorKey<S> | Selector<S, T>) => {
    try{
      const context = useContext(this.Context);
      return (context?.useNeuron<T, A>(selector as any));
    }catch(err){
      console.error(console.error(`NeuronGSM: Protected store hooks cannot be called outside of protected store scope.`), err);
    }
  }

  useDispatch = <T = unknown, D = {[key: string]: unknown}>(selector: Core.SelectorKey<S>) => {
    try{
      const context = useContext(this.Context);
      return ((mutator: DispatchMutator<T, S, D>) => context?.useDispatch<T>(selector, mutator as any));
    }catch(err){
      console.error(console.error(`NeuronGSM: Protected store hooks cannot be called outside of scope.`), err);
    }
  };
