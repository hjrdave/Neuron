...
describe("Get Slice from state item.", () => {
    ...
    it("Get Person data.", () => {
        ...
        expect(getSlice((store) => store.person.name, Store)).toBe(StateValues.person.name);
        expect(getSlice((store) => store.person.age, Store)).toBe(StateValues.person.age);
        expect(getSlice((store) => store.person.job.salary, Store)).toBe(StateValues.person.job.salary);
    });
    it("set Person data.", () => {
        expect(Store.has(StateKeys.Person)).toBeTruthy();
