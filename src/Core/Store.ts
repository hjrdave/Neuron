...
import { StoreItem, SelectorKey, Features, Actions, StoreProps, StateType, DataProps, ActionProps, DispatchCallback, InterceptorTypes, DispatchMutator, GetState, SetState, GetStore, AddState, DispatchPayload, OnDispatch, UseModule, GetActions, HasState, ResetState } from "./Core.interfaces";
...
export default class Store<S = StoreProps> implements IStore<S> {
    ...
    private actionsInventory: Map<SelectorKey<S>, Actions<ActionProps, StateType, S>>;
    ...
    private dispatchPayload = <T = StateType, D = DataProps>(payload: IPayload<T, S, D>, type?: InterceptorTypes) => {
        const interceptor = new Interceptor<T, S, D>({
             payload: payload, 
             modules: this.moduleInventory
         });
         (type === InterceptorTypes.OnLoad) ? (interceptor.onload()) : interceptor.onRun();
         if(interceptor.isStateNotPrevState_PrimitiveCheckOnly() && interceptor.isStateNotNullOrUndefined()){
     ...
     private createPayload = <T = StateType, D = DataProps>(params: StoreItem<T, S>) => {
     ...
     */
    readonly use = (module: IModule<StateType, S>) => (this.moduleInventory.set(module.name, module));
     ...
     */
    readonly add = <T=StateType, A = ActionProps>({ key, state, actions, features }: StoreItem<T, S, A>) => {
        if(!this.stateInventory.has(key)){
            ...
            (features) ? this.featureInventory.set(payload.key, features as Features<unknown, S>) : null;
            (actions) ? this.actionsInventory.set(payload.key, actions as Actions<ActionProps, unknown, S>) : null;
     ...
     */
    readonly get = <T = StateType>(selector: SelectorKey<S>) => (this.stateInventory.get(selector) as T)

    /**
     * Get actions from a store item. 
     * @param {string} selector - key of the store item you want to select.
     */
    readonly getActions = <A = ActionProps, T = StateType>(selector: SelectorKey<S>) => {
        const dispatch = (mutator: DispatchMutator<T, S>) => this.dispatch(selector, mutator);
     ...
     */
    readonly set = <T = StateType>(selector: SelectorKey<S>, newState: T | ((prevState: T) => T)) => { 
        if(this.stateInventory.has(selector)){
            const prevState = this.get<T>(selector),
            payload = this.createPayload<T>({
                key: selector,
                state: ((typeof newState === 'function') ? (newState as (prevState: T) => T)?.(prevState) : newState) as T
            })
     ...
     */
    readonly reset = (key?: SelectorKey<S>) => (key) ? this.set(key, this.initialStateInventory.get(key)) : (
        this.initialStateInventory.forEach((_, key) => (this.set(key, this.initialStateInventory.get(key))))
    )
     ...
     */
    readonly dispatch = <T = StateType, D = DataProps>(key: SelectorKey<S>, mutator: DispatchMutator<T, S, D>) => {
    ...
    readonly onDispatch = (callback: DispatchCallback<S>) => {
        ...
        this.stateInventory.forEach((_, key) => this.dispatcher.listen(key, callback));
    }
