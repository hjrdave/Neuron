...
import { SelectorKey as TSelectorKey, Actions as TActions, StateType, StoreProps, DataProps, DispatchMutator as TDispatchMutator, DispatchPayload as TDispatchPayload} from "./Core.interfaces";
...
namespace NeuronGSM{
     ...
     */
    export function Module<T = unknown, S = {[key:string]: unknown}>(params: Params<T, S>){
    ...
    export type Payload<T = unknown, S = {[key:string]: unknown}> = IPayload<T, S>;
    export type Actions<A = {[key:string]: unknown}, T = unknown, S = {[key:string]: unknown}> = TActions<A, T, S>;
    export type DispatchMutator <T = StateType, S = StoreProps, D = DataProps> = TDispatchMutator<T, S, D>;
